...
import cloudinary from '../../../../Usuarios/src/infrastructure/cloudinaryConfig';
...
export class QuejaController {
    ...
    crearQueja = async (req: Request, res: Response) => {
        try {
            ...
            await queja.save();
            // Registrar auditoría
            await logAudit(queja._id.toString(), 'create', `Queja creada: ${title}`);
    ...
    actualizarQueja = async (req: Request, res: Response) => {
        ...
        try {
            ...
            await queja.save();
            // Registrar auditoría
            await logAudit(queja._id.toString(), 'update', `Queja actualizada: ${queja.title}`);
            res.status(200).send(queja);
        } catch (error) {
...
import { getChannel } from '../../../../Usuarios/src/services/rabbit/rabbitmq';
...
export class QuejaController {
    ...
    crearQueja = async (req: Request, res: Response) => {
        try {
            const { title, description, category, likes, usersLiked } = req.body;
            ...
            const queja = new Queja({ title, description, category, imageUrl, userId, likes, usersLiked });
    ...
    private async enviarCorreoConAdjunto(queja: Queja) {
        const categoriasPorCorreo: Record<string, string[]> = {
            'cristianrmartist@gmail.com': ['alumbrado'],
            '221267@ids.upchiapas.edu.mx': ['limpieza'],
            'cristiangv1313@gmail.com': ['baches'],
        ...
        for (const destinatario of destinatarios) {
            const mailOptions = {
                ...
                attachments: [
                    {
                        filename: `queja_${queja.id}.docx`,
                        path: `./reportes/queja_${queja.id}.docx`,
                    },
                ],
            };
    ...
    //generar un archivo Word
    private async generarWord(queja: Queja) {
        const doc = new Document({
            sections: [
                {
                    children: [
        ...
        // Guardar el archivo Word
        const buffer = await Packer.toBuffer(doc);
        fs.writeFileSync(`./reportes/queja_${queja.id}.docx`, buffer);
    }
    ...
    obtenerQuejasPorUsuario = async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (!quejas.length) {
                return res.status(404).send({ message: 'No se encontraron quejas para este usuario.' });
            }
    ...
    obtenerQuejaPorId = async (req: Request, res: Response) => {
        const _id = req.params.id;
        try {
            const queja = await Queja.findByPk(_id);
            if (!queja) {
                return res.status(404).send({ error: 'Queja no encontrada.' });
            }
            res.status(200).send(queja);
    ...
    obtenerQuejasPorCategoriaAgrupadas = async (req: Request, res: Response) => {
        try {
            const quejasAgrupadas = await Queja.findAll({
    ...
    obtenerQuejasPorFecha = async (req: Request, res: Response) => {
    try {
        const quejasPorFecha = await Queja.findAll({
            attributes: [
                [sequelize.fn('DATE', sequelize.col('dateCreated')), 'date'],
                [sequelize.fn('COUNT', sequelize.col('id')), 'count'],
            ],
            group: [sequelize.fn('DATE', sequelize.col('dateCreated'))],
            order: [[sequelize.fn('DATE', sequelize.col('dateCreated')), 'ASC']],
        });
...
/*     obtenerUsuariosConMasQuejas = async (req: Request, res: Response) => {
    ...
    // Actualizar una queja por ID
    actualizarQueja = async (req: Request, res: Response) => {
        const updates = Object.keys(req.body) as Array<keyof Queja>;
        const allowedUpdates: Array<keyof Queja> = ['title', 'description', 'category', 'status'];
        const isValidOperation = updates.every((update) => allowedUpdates.includes(update));
        ...
        try {
            ...
            if (req.body.status && req.body.status !== queja.status) {
                try {
                    ...
                    console.log('Registro histórico guardado.');
                } catch (error) {
                    console.error('Error al guardar registro histórico:', error);
                }
            }
            ...
            // Registrar auditoría
            await logAudit(queja.id.toString(), 'update', `Queja actualizada: ${queja.title}`);
            res.status(200).send(queja);
        } catch (error) {
    ...
    darLike = async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (queja.usersLiked && queja.usersLiked.includes(userId)) {
                return res.status(400).json({ message: 'Ya has dado like a esta queja' });
            }
    ...
    eliminarQueja = async (req: Request, res: Response) => {
        try {
            ...
            await logAudit(queja.id.toString(), 'delete', `Queja eliminada: ${queja.title}`);
    
            res.status(200).send({ message: 'Queja eliminada correctamente', queja });
        } catch (error) {
